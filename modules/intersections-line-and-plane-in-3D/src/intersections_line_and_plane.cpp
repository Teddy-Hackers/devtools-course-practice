...
std::pair<__int8, std::vector<double>> isIntersection(std::vector<double> plane,
    ...
    if (B != 0) {
        ...
        return std::pair<__int8, std::vector<double>>(0, result);
    }else {
        if (A == 0) {
            // a straight line belongs a plane
            return std::pair<__int8, std::vector<double>>(1, std::vector<double>(0));
        }else {
            // a straight line parallel a plane
            return std::pair<__int8, std::vector<double>>(2, std::vector<double>(0));
        }
    }
    return std::pair<__int8,
