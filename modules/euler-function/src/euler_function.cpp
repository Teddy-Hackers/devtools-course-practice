...
int euler_function(int value) {
  ...
  // If value is composite number, then its divisor belongs to the segment [2,
  // floor(sqrt(value))]. If value is prime number,     then the body of the
  // loop will not be executed. Enumerate all integers from 2 to sqrt(value).
  for (int number = 2; number * number <= value; ++number) {
    ...
    if (value % number == 0) {
      ...
      numberOfCoprimeNumbers *=
          (unsigned long long)(pow(number, n) - pow(number, n - 1));
    }
  }
