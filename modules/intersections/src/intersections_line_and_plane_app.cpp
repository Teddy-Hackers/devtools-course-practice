...
std::string I3DApplication::operator()(int argc, const char** argv) {
    if (argc == 1)
        help(argv[0]);
    char* ptr;
    std::vector<double>plane = { (double)strtol(argv[1], &ptr, 10),
                                 (double)strtol(argv[2], &ptr, 10),
                                 (double)strtol(argv[3], &ptr, 10),
                                 (double)strtol(argv[4], &ptr, 10) };
    std::vector<double>dot = { (double)strtol(argv[5], &ptr, 10),
                               (double)strtol(argv[6], &ptr, 10),
                               (double)strtol(argv[7], &ptr, 10) };
    std::vector<double>direction = { (double)strtol(argv[8], &ptr, 10),
                                     (double)strtol(argv[9], &ptr, 10),
                                     (double)strtol(argv[10], &ptr, 10) };
    try {
