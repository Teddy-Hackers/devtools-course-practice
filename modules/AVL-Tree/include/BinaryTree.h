 ...
 public:
  ...
  bool operator==(const BinaryTree<TData, TKey>& N) const {
    ...
    if (N.Root != nullptr) {
      ...
      if (Root != nullptr) {
        C = FindMin(Root);
        while (C->GetKey() != FindMax(Root)->GetKey() &&
               CN->GetKey() != FindMax(N.Root)->GetKey()) {
          if (CN->GetKey() != C->GetKey()) {
            return 0;
          }
          CN = FindNext(CN);
          C = FindNext(C);
...
template <typename TData, typename TKey>
TreeNode<TData, TKey>* BinaryTree<TData, TKey>::FindMin(
    TreeNode<TData, TKey>* node) const {
...
template <typename TData, typename TKey>
TreeNode<TData, TKey>* BinaryTree<TData, TKey>::FindMax(
    TreeNode<TData, TKey>* node) const {
  TreeNode<TData, TKey>* tmp = node;
...
template <typename TData, typename TKey>
TreeNode<TData, TKey>* BinaryTree<TData, TKey>::FindPrev(
    TreeNode<TData, TKey>* node) const {
...
template <typename TData, typename TKey>
TreeNode<TData, TKey>* BinaryTree<TData, TKey>::FindNext(
    TreeNode<TData, TKey>* node) const {
  TreeNode<TData, TKey>* res = nullptr;
...
void BinaryTree<TData, TKey>::Delete(TKey k) {
  ...
  if (z != nullptr) {
    TreeNode<TData, TKey>* y =
        (z->Left != nullptr && z->Right != nullptr) ? FindNext(z) : z;
