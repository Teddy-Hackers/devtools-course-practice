...
#ifndef MODULES_LIST_INCLUDE_LIST_H_
#define MODULES_LIST_INCLUDE_LIST_H_
...
public:
	...
	void InsertAfter(Node < T >* node,
		const T& d); // вставить элемент d после звена node
	void Delete(const T& d); // удалить звено со значением data = d
	Node < T >* Search(
		const T& d); // найти указатель на звено со значением data = d
	...
	Node < T >* returnFictitiousNode() {
		return head
	} //для проверки, что фиктивное звено действительно есть и оно такое, какое
	//было создано.
};
...
#endif // MODULES_LIST_INCLUDE_LIST_H_
