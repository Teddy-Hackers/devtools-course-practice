...
TEST(polynomial_calculator, parser_and_lexem_no_throw_recognizes_repetitions_of_signs) {
    polynomial_calculator P;
...
TEST(polynomial_calculator, parser_and_lexem_no_throw_recording_test_on_big_polynomials) {
    polynomial_calculator P;
...
TEST(polynomial_calculator, parser_and_lexem_no_throw_recognizes_multiplication) {
    polynomial_calculator P;
...
TEST(polynomial_calculator, parser_and_lexem_no_recording_test_on_big_polynomials_2) {
    polynomial_calculator P;
...
TEST(polynomial_calculator, polynomial_calculator_can_Detect_Wrong_Operation_Format) {
    polynomial_calculator P;
    ASSERT_ANY_THROW(P.parser("2x^yz"));
}

TEST(polynomial_calculator, parser_and_lexem_unidentified_characters) {
    polynomial_calculator P;
    ASSERT_ANY_THROW(P.parser("2x^2 &+5x"));
}

TEST(polynomial_calculator, polynomial_calculator_can_Detect_Wrong_Number_Format) {
    polynomial_calculator P;
    ASSERT_ANY_THROW(P.parser("2x^5+/3x^4 -2.6 x^3+x^2$$+x- 8.6"));
}

TEST(polynomial_calculator, equality_operator_test_compares_the_same) {
    polynomial_calculator P("2x^2yz^3+5x^3z^5y");
    polynomial_calculator Q("5yz^5x^3+2x^2z^3y");
    ASSERT_EQ(Q, P);
}

TEST(polynomial_calculator, equality_operator_test_compares_different) {
...
TEST(polynomial_calculator, addition_operation_if_one_polynom_is_zero) {
...
TEST(polynomial_calculator, minus_operation_if_one_polynom_is_zero) {
    polynomial_calculator P("2xy^2");
    polynomial_calculator Q("2xy-2xy");
    Q.cancellation();
    polynomial_calculator Res("-2xy^2");
...
TEST(polynomial_calculator, minus_operation_if_one_polynom_is_zero_2) {
    polynomial_calculator P("2xy^2");
...
TEST(polynomial_calculator, multi_operation_if_one_polynom_is_zero) {
    polynomial_calculator P("2xy^2");
    polynomial_calculator Q("0");
    Q.cancellation();
