...
Graph::Graph(const std::unordered_map<int, std::list<int>>& adjacency_map, uint64_t num_of_nodes) {
	if (num_of_nodes == 0)throw "Size can't be zero";
	__num_of_nodes = num_of_nodes;
...
void Graph::Add_Edge(uint64_t u, uint64_t w) {
	if (u < 0 || u >= __num_of_nodes || w < 0 || w >= __num_of_nodes)throw "Nodes are numbered from 0 to __num_of_nodes-1!!!";
	if (!Has_Edge(u, w)) {
...
void Graph::Depth_Search(uint64_t start_node,bool* visited) {
	...
	for (auto it = adjacency_map[start_node].begin(); it != adjacency_map[start_node].end(); ++it) {
...
uint64_t Graph::Get_Num_Of_Components() {
	...
	for (int i = 0; i < __num_of_nodes; i++) {
		visited[i] = false;
	}
